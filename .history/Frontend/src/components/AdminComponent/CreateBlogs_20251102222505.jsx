  ...
  const handleSubmit= async(e)=>
  {
    e.preventDefault();
      ...
      if (formData.AddImage) {
        form.append("AddImage", formData.AddImage);
      }
    try {
      const res = await axiosInstance.post("/createBlog", form, {
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex justify-center items-center flex-col mt-10 px-4">
        <form
          className="bg-white flex flex-col gap-8 p-10 shadow-2xl rounded-2xl w-full max-w-3xl border border-gray-100"
        ...
        >
          ...
          {/* Submit Button */}
            ...
            >
              {loading ? "Submitting..." : "Submit Blog"} {/* button text changes */}
            </button>
          </div>
        </form>
      </div>
