  ...
  return (
    <div>
      <div className="sm:hidden px-4 ">
        <button
          className="p-2 bg-yellow-500 mt-2 rounded-xl"
          onClick={() => setNav(!nav)}
        >
          <MenuIcon />
        </button>
      ...
      {nav && (
        <div className="fixed top-0 left-0 w-screen h-screen bg-yellow-500 z-50 flex flex-col items-center justify-center gap-6">
          {items.map((data, index) => (
            <a
              key={index}
              href={data.link}
              onClick={() => setNav(false)} // closes menu when clicked
          ...
          <button
            ...
            className="p-2 bg-yellow-500 mt-2 rounded-xl top-4 left-4 text-3xl text-white"
          >
            ✕
          </button>
        </div>
