  ...
  return (
    <div>
      <div className="sm:hidden px-4 ">
        <button className="p-2 bg-yellow-500 mt-2 rounded-xl"
        onClick={()=>(setNav(!nav))}><MenuIcon/></button>
      </div>
      {
        nav && <div className="h-screen w-full bg-yellow-500 sticky">
          {items.map((data, index)=>
          (
            <a href={data.link}>test</a>
          ))}
        </div>
      }
