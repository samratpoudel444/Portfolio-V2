...
const createExperince= async(req, res, next)=>
{
    try{
        ...
        if(!CompanyName | !WorkedYear || !Position)
        {
         ...
         const query = `Insert into "experinceTables" ("CompanyName", "WorkedYear", "Position") Values (:CompanyName,:WorkedYear, :Position )`;
         ...
         return res
           .status(201)
           .json({ message: "Experince Info Inserted Sucessfully" });
    }
    catch(err)
    {
