const { cloudinary } = require("../../helper/cloudinaryHelper");
const fs = require("fs");
const sequelize = require("../../helper/sequelizeHelper");

const UpdateCv = async (req, res, next) => {
  try {
    if (!req.file) {
      return next({ code: 400, message: "Please provide the Image" });
    }

    const [data]= await sequelize.query({})

    const filePath = req.file.path;
    const uploadImage = await cloudinary.uploader.upload(filePath, {
      folder: "CV_images",resource_type: "raw",
    });

    fs.unlink(filePath, (err) => {
      if (err) {
        console.log("Error");
      }
    });
    const AddImage = uploadImage.secure_url;
    const query = `Update "CVTable" set "CVLink"= :AddImage where id= :id `;

    const insertCV = await sequelize.query(query, {
      replacements: {
        AddImage,
        id:1
      },
    });
    return res.status(201).json({ message: "CV Updated Sucessfully" });
  } catch (err) {
    console.log(err);
    return next({
      code: err.code || 500,
      message: err.message || "Internal Server Error",
    });
  }
};

module.exports = UpdateCv;
